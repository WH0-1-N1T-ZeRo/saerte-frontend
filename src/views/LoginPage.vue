  ...
  <div class="flex flex-col items-center min-h-screen bg-white p-4">
    <div class="w-full max-w-md py-8">
      ...
      <div class="flex justify-between items-center mb-6">
        <button class="focus:outline-none" @click="$router.go(-1)"> <svg xmlns="http://www.w3.org/2000/svg"
      ...
      <div v-if="!otpSent">
        ...
        <div class="mb-4">
          ...
          <div class="flex border border-gray-300 rounded-md focus-within:ring-2 focus-within:ring-red-500">
            ...
            <input type="tel" id="phone" v-model="phoneNumber" placeholder="08xxxxxxxxx"
      ...
      <div v-if="otpSent">
        ...
        <p class="text-gray-600 text-center text-sm mb-4 flex items-center justify-center">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-500 ml-2" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
        ...
        <div class="mb-4">
          <div class="flex justify-around space-x-2">
            <input v-for="index in 6" :key="index" :ref="`otpInput${index - 1}`" type="tel"
              v-model="otpDigits[index - 1]" @input="handleOtpInput(index - 1)"
              @keydown.backspace="handleBackspace(index - 1)" maxlength="1"
        ...
        <button @click="verifyOtp" :disabled="otpCode.length !== 6 || isLoading"
          class="w-full bg-red-500 text-white py-3 rounded-md text-sm font-semibold focus:outline-none hover:bg-red-600 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center">
        ...
        <p class="mt-4 text-gray-500 text-xs text-center">
          Didn't receive OTP?
          <button @click="resendOtp" :disabled="resendTimer > 0 || isLoading"
...
export default {
  ...
  computed: {
    isValidPhoneNumber() {
      ...
      return /^\d{10,13}$/.test(this.phoneNumber);
    },
  ...
  methods: {
    async requestOtp() {
      ...
      try {
        ...
        const response = await api.post('/request-otp', { "telepon": this.phoneNumber.toString() });
    ...
    async verifyOtp() {
      ...
      try {
        ...
        const response = await api.post('/verify-otp', {
          "telepon": this.phoneNumber.toString(),
          "otp": this.otpCode.toString()
        });
    ...
    async resendOtp() {
      ...
      try {
        ...
        const response = await api.post('/request-otp', { "telepon": this.phoneNumber.toString() });
        lert(response.data.message || 'OTP berhasil dikirim ulang!');
        this.startResendTimer();
